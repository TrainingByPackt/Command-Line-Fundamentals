...
function show_nth_game()
{
  ...
  do
    ...
    # Print an error message (remember count is 1 more than the last game)
    if ! read_chunk $should_print 
    then
      echo -e "Invalid game index - maximum is $(( count - 1 )) \n"
      return 
    fi
...
function show_games_won_in()
{
  ...
  do
    ...
    then
      ...
      # Also increment game_count to keep track if there were any games at all
      if (( num_moves <= $1 )) 
      then
        (( ++game_count ))
        echo "Game $index: $num_moves moves" 
        echo "$moves"  
        echo
      fi   
  ...
  # Print message if no games were found
